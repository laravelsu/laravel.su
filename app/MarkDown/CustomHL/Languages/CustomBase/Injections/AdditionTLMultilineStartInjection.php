...
#[Before]
final readonly class AdditionTLMultilineStartInjection implements Injection
{
    public function parse(string $content, Highlighter $highlighter): ParsedInjection
    {
        preg_match_all('/(?<match>(.)*\/\/ \[tl\! add\:start\])/', $content, $matches, PREG_OFFSET_CAPTURE);  //  /(?<match>(.)*\/\/ \[tl\! add\])/
